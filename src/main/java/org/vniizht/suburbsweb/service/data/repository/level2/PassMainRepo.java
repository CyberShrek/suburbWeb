...
public interface PassMainRepo extends Level2Repo<PassMain, Long> {
    ...
    @Query(value = "SELECT main.idnum FROM prig_bekap.l2_pass_main main " +
...
//            "AND main.f_r10af3[8] = true", nativeQuery = true)
    ...
    @Query(value = "SELECT main.* FROM prig_bekap.l2_pass_main main " +
            "LEFT JOIN prig_bekap.l2_pass_main_upd upd  ON (main.idnum = upd.idnum) " +
            "LEFT JOIN prig_bekap.l2_pass_ex ex         ON (main.idnum = ex.idnum AND ex.npp = 1) " +
            "LEFT JOIN prig_bekap.l2_pass_refund refund ON (main.idnum = refund.idnum) " +
            "LEFT JOIN prig_bekap.l2_pass_cost costs    ON (main.idnum = costs.idnum)" +
